...
public class StepGenerator {
    ...
    // crée une ligne de notes, avec de nombreuses vérifications, équilibrages et
    // filtrages
    ...
    private static void makeNoteLine(String lastLine, float time, int steps, int holds, boolean mines,
        ...
        if (steps > 1 && time - lastJumpTime < (mines ? 2f : 4f))
            steps = 1; // ne pas spammer les sauts
        if (steps >= 2) {
            // pas de mains (hands)
            steps = 2;
            lastJumpTime = time;
        ...
        if (holds + currentHoldCount > 2)
            holds = 2 - currentHoldCount;
        if (steps + currentHoldCount > 2)
            steps = 2 - currentHoldCount;
        // si on a eu une série de 3 holds, on n'en crée pas de nouveau pour éviter que
        // le joueur ne tourne sur lui-même
        if (holdRun >= 2 && holds > 0)
            holds = 0;
        // est-ce qu'on arrête les holds ?
        char[] noteLine = getHoldStops(currentHoldCount, time, holds);
        // si on fait un pas, mais qu'on vient de finir un hold, on déplace cette fin de
        // hold vers le haut pour laisser le temps
        ...
        do {
            ...
            while (stepcount > 0) {
                ...
                if (noteLine[stepindex] != EMPTY || holding[stepindex] > 0f)
                    continue;
                if (holdcount > 0) {
        ...
        if (willhold[0] > holding[0])
            holding[0] = willhold[0];
        if (willhold[1] > holding[1])
            holding[1] = willhold[1];
        if (willhold[2] > holding[2])
            holding[2] = willhold[2];
        if (willhold[3] > holding[3])
            holding[3] = willhold[3];
        if (getHoldCount() == 0) {
            holdRun = 0;
        } else
            holdRun++;
    ...
    private static float getFFT(float time, TFloatArrayList FFTAmounts, float timePerFFT) {
        ...
        if (index < 0 || index >= FFTAmounts.size())
            return 0f;
        return FFTAmounts.getQuick(index);
    }

    private static boolean sustainedFFT(float startTime, float len, float granularity, float timePerFFT,
    ...
    public static String GenerateNotes(int stepGranularity, int skipChance,
        ...
        for (float t = timeOffset; t <= totalTime; t += timeGranularity) {
            ...
            if (t > 0f) {
                ...
                boolean sustained = sustainedFFT(t, 0.75f, timeGranularity, timePerFFT, FFTMaxes, FFTAverages, 0.25f,
                        0.45f);
                ...
                if (nearKick && (nearSnare || nearEnergy) && timeIndex % 2 == 0 &&
                    ...
                    // on ne saute que dans les zones intenses, sur les battements complets (pas de
                    // demi-temps)
                    steps = 2;
                }
            ...
            if (AutoStepper.DEBUG_STEPS) {
                ...
                makeNoteLine(lastLine, t, steps, holds, allowMines, FFTMaxes, timePerFFT);
    ...
    static boolean shouldPlaceMine(float time, TFloatArrayList FFTMaxes, float timePerFFT) {
        ...
        if (idx < 0 || idx >= FFTMaxes.size())
            return false;
        float energy = FFTMaxes.getQuick(idx);
        // Plus l'énergie est forte, plus la probabilité de mine est élevée
        if (energy > 0.8f)
            return rand.nextInt(3) == 0; // 33% en zone très intense
        if (energy > 0.5f)
            return rand.nextInt(6) == 0; // 17% en zone intense
        if (energy > 0.3f)
            return rand.nextInt(12) == 0; // 8% en zone moyenne
